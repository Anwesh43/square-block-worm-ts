...
class SBWNode {
    ...
    addNeighbor() {
        if (this.i < nodes - 1) {
            this.next = new SBWNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context : CanvasRenderingContext2D) {
        drawSBWNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : SBWNode {
        var curr : SBWNode = this.prev
        if (dir == 1) {
            curr = this.next
        }
        if (curr) {
...
class SquareBlockWorm {

    curr : SBWNode = new SBWNode(0)
    dir : number = 1
